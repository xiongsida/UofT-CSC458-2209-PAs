...
def start_iperf(net):
    ...
    h1 = net.get('h1')
    h1.cmd("iperf -c {} -t {}".format(h2.IP(), args.time))
...
def start_ping(net):
    ...
    h2 = net.get("h2")
    popen = h1.popen("ping -c {} -i 0.1 {} > {}/ping.txt".format(args.time * 10, h2.IP(), args.dir), shell=True)
    popen.communicate()
...
def bufferbloat():
    ...
    def transfer_time(net):
        ...
        time = float(h2.popen(fetch).communicate()[0])
        return float(time)
